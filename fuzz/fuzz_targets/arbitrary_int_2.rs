...
use varing::{arbitrary_int::v2::*, consume_varint, Varint};
...
macro_rules! gen_signed_storage {
    ($($storage:ident($start:literal..=$end:literal)),+$(,)?) => {
        $(
            seq_macro::seq!(
                N in $start..=$end {
                    ...
                    impl<'a> arbitrary::Arbitrary<'a> for $storage {
                        fn arbitrary(g: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
                            Ok(match g.int_in_range($start..=$end)? {
                                #(
                                    N => Self::I~N(i~N::new(g.int_in_range(i~N::MIN.value()..=i~N::MAX.value())?)),
                                )*
                                _ => unreachable!(),
                            })
                        }
                    ...
                    impl $storage {
                        fn check(self) {
                            match self {
                                #(
                                    Self::I~N(value) => check_i~N(value),
                                )*
                            }
                        }
                    }
...
impl Types {
  fn check(self) {
    match self {
      ...
      Self::I8(value) => value.check(),
      Self::I16(value) => value.check(),
      Self::I32(value) => value.check(),
      Self::I64(value) => value.check(),
      Self::I128(value) => value.check(),
    }
  }
}
